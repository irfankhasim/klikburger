...
export const writeTools = [
  ...
  // ── 3. add_purchase ───────────────────────────────────────────────────────
  {
    ...
    async handler(input) {
      ...
      const cpu = +input.costPerUnit;
      const totalAmount = +(qty * cpu).toFixed(2);
      ...
      const purchaseDoc = {
        createdAt: ts(),
        totalAmount,
        supplier: String(input.supplierName || '').slice(0, 80),
        notes: [input.notes, input.invoiceNo ? `Inv: ${input.invoiceNo}` : ''].filter(Boolean).join(' · ').slice(0, 200),
        lines: [{
          ingredientId: input.ingredientId,
          label: input.ingredientName,
          qty,
          unit: input.unit,
          unitCost: cpu,
      ...
      const batchDoc = {
        ...
        qtyOriginal: qty,
        costPerUnit: cpu,
        purchaseTotalRm: totalAmount,
        purchaseUnit: input.unit,
        openedAt: ts(),
        purchaseOccurredAt: ts(),
      ...
      const ledgerDoc = {
        ...
        unit: input.unit,
        costPerUnit: cpu,
        nameSnapshot: input.ingredientName,
        notes: input.notes ?? '',
        createdAt: ts(),
      };
      ...
      await auditLog({
        ...
        payload: { ingredientId: input.ingredientId, quantity: qty, totalAmount },
